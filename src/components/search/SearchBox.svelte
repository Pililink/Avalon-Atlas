<script lang="ts">
  ...
  import MapListItem from "../map/MapListItem.svelte";
  ...
  async function handleInput() {
    ...
    showDropdown = true;
    try {
      const res = await callCommand<SearchResult[]>("search_maps", { query, maxResults: 10 });
      results = res;
    } catch (e) {
  ...
  export async function handleCapture() {
    ...
    showDropdown = false;
    try {
      const res = await callCommand<SearchResult[]>("capture_and_search");
      if (res.length > 0) {
